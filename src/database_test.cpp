...
void TestDatabaseFind() {
	{
		const multimap<Date, string> expected = {
			{{2017, 11, 17}, "Friday"},
			{{2017, 11, 18}, "Saturday"},
			{{2017, 11, 19}, "Sunday"}
		};
	...
	}
	{
		...
		const multimap<Date, string> expected = { {{2014, 11, 17}, "Monday"}, {{2016, 11, 17}, "Thursday"} };
	...
	}
	{
		...
		AssertEqual(db.FindIf(acceptsNothing), multimap<Date, string>(), "Database find: accepts nothing");
	}
}
...
void TestDatabaseRemove() {
	{
		...
		auto alwaysTrue = [](const Date&, const string&) { return true; };
		AssertEqual(db.RemoveIf(alwaysTrue), 4u, "Database remove: alwaysTrue removes all 1");
		AssertEqual(db.FindIf(alwaysTrue), multimap<Date, string>(), "Database remove: alwaysTrue removes all 2");
	}
	{
		Database db;
		db.Add({ 2017, 11, 17 }, "Friday");
		...
		AssertEqual(db.RemoveIf(complexCondition), 2u, "Database remove: complex condition 1");
		AssertEqual(db.FindIf(complexCondition), multimap<Date, string>(), "Database remove: complex condition 2");
	}
	{
